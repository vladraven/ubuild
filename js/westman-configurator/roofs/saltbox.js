export default {
    getWallShape: (w, h, isGable, wallId, params, THREE) => {
        ...
        shape.lineTo(w/2, h);
        if (isGable) {
            const peak = (params.width * 0.66) * (params.pitch / 12);
            const tY = params.thickness / Math.cos(Math.atan(peak / (params.width * 0.66)));
            const ridgeX = (params.width / 2) - (params.width * 0.33);
            shape.lineTo(ridgeX, h + peak - tY);
        }
        shape.lineTo(-w/2, h); 
        shape.closePath();
        return shape;
    ...
    getRoofData: (params, THREE) => {
        ...
        const pts = [
            ...
            new THREE.Vector3(ridgeX, hP, d2), 
            new THREE.Vector3(w2, hB, d2),
            new THREE.Vector3(w2, hB, -d2), 
            new THREE.Vector3(ridgeX, hP, -d2), 
            new THREE.Vector3(-w2, hB, -d2)
