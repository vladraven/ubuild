...
function createGutter(length) {
    ...
    const w =
        0.14 +
        GUTTER_CONFIG.gutter.widthOffset;
    ...
    const safeLength =
        Math.max(
            0.01,
            length +
            GUTTER_CONFIG.gutter.lengthOffset
        );
...
export function createGuttersGroup(
    ...
    const gutterOffsetY =
        gData.config.gutterOffsetY;

    const gutterL =
        createGutter(gData.length);

    gutterL.scale.x = -1;

    gutterL.position.set(
        gData.eaves.left.x +
            GUTTER_CONFIG.gutter.offsetX,
    ...
    gData.downspouts.forEach(
        dsData => {
            const ds =
                createDownspout(dsData);
