...
function resolveMaterial(context) {
    ...
    if (typeof context.materials.get === 'function') {
        const mat = context.materials.get('roofMetal', context.colors?.roof, textureBundle);
...
function createPlaneGroup(planeId, panels, material) {
    ...
    for (const panel of panels) {
        group.add(createPanelMesh(panel, material));
    }
    return group;
}
...
function createObject(context) {
    ...
    if (context.model?.visibility?.roof === false) {
        return root;
    }
