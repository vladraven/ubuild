...
function collectSharedTextures() {
    const textures = new Set();
...
function disposeLocalMaterial(material, sharedTextures) {
...
function disposeObjectTree(node) {
    ...
    const sharedTextures = collectSharedTextures();
...
function readMetricValue(id, fallback = 0) {
    ...
    const value = parseFloat(
        element.getAttribute('data-current-m')
    );
...
function readBuildingParameters() {
    ...
    return {
        width: readMetricValue(
            'inputW',
            bc.max_width || 18.288
        ),

        length: readMetricValue(
            'inputL',
            bc.max_length || 30.48
        ),

        height: readMetricValue(
            'inputH',
            bc.max_height || 4.8768
        ),
        ...
        wsEnabled:
            document.getElementById('wainscotEn')?.checked ||
            false,
        ...
        intLinerEn:
            document.getElementById('intWallsEn')?.checked ||
            false,

        intLinerH:
            parseFloat(
                document.getElementById('intWallsH')?.value || 100
            ),

        mezzEn:
            document.getElementById('mezzEn')?.checked ||
            false,

        mezzCov:
            document.getElementById('mezzCov')?.value ||
            '1',

        mezzZ:
            parseFloat(
                document.getElementById('mezzZ')?.value || 0
            ),

        mezzH:
            parseFloat(
                document.getElementById('mezzH')?.value || 50
            ),

        mezzColor:
            document.getElementById('colorMezzanine')?.value,

        craneEn:
            document.getElementById('craneEn')?.checked ||
            false,

        craneZ:
            parseFloat(
                document.getElementById('craneZ')?.value || 50
            ),

        drivewayEn:
            document.getElementById('drivewayEn')?.checked ??
            false
    };
}
...
export function updateBuilding() {
    ...
    mainGroup.add(
        createMainFramesGroup(geometry)
    );
    ...
    mainGroup.add(
        createAwningsGroup(geometry)
    );

    mainGroup.add(
        createWainscotGroup(geometry)
    );
    ...
    const checkTrims =
        document.getElementById('checkTrims')?.checked ??
        true;

    const checkGutters =
        document.getElementById('checkGutters')?.checked ??
        false;
    ...
    if (
        checkTrims &&
        geometry.roof.type === 'gabled'
    ) {
        mainGroup.add(
            createRidgeGroup(geometry)
        );
    }
    ...
    mainGroup.add(
        createGirtsGroup(
            geometry,
            document.getElementById('checkGirts')?.checked ??
                true
        )
    );
    ...
    mainGroup.add(
        createPurlinsGroup(
            geometry,
            document.getElementById('checkPurlins')?.checked ??
                true
        )
    );
    ...
    mainGroup.add(
        createEndWallColumnsGroup(
            geometry,
            document.getElementById('checkEWColumns')?.checked ??
                true
        )
    );
...
function updateSidebarSummary(
    ...
    const wallProfileSelect =
        document.getElementById('wallProfile');

    const roofColorSelect =
        document.getElementById('colorRoof');

    const wallColorSelect =
        document.getElementById('colorWall');
    ...
    if (dimsEl) {
        ...
        const w =
            (widthM * mult).toFixed(0);

        const l =
            (lengthM * mult).toFixed(0);

        const h =
            (heightM * mult).toFixed(0);
