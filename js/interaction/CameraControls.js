...
export function createCameraControls({ camera, domElement, onUpdate }) {
    if (!camera || !domElement) {
        throw new TypeError('Camera and DOM element are required for CameraControls');
    }
    ...
    function onWheel(e) {
        ...
        spherical.radius = Math.max(2.0, Math.min(1500.0, spherical.radius * factor));
        updateCameraPosition();
    }
    ...
    function frameBounds(bounds) {
        ...
        const fitDistance = (maxDim / 2.0) / Math.tan(fovRad / 2.0);
