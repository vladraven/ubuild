...
function validateDimensions(model) {
    assertRange(model.dimensions.width, LIMITS.width.min, LIMITS.width.max, 'dimensions.width');
    assertRange(model.dimensions.length, LIMITS.length.min, LIMITS.length.max, 'dimensions.length');
    assertRange(model.dimensions.height, LIMITS.height.min, LIMITS.height.max, 'dimensions.height');
}

function validateRoof(model) {
    if (!ROOF_TYPES.includes(model.roof.type)) {
        throw new RangeError(`Unsupported roof type: ${model.roof.type}`);
    }
    assertFinite(model.roof.pitchRatio, 'roof.pitchRatio');
    if (model.roof.pitchRatio <= 0) {
        throw new RangeError('roof.pitchRatio must be greater than zero');
    }
    for (const side of ['front', 'back', 'left', 'right']) {
        assertRange(model.roof.overhangs[side], LIMITS.overhang.min, LIMITS.overhang.max, `roof.overhangs.${side}`);
    }
}

function validateWalls(model) {
    assertRange(model.walls.thickness, LIMITS.wallThickness.min, LIMITS.wallThickness.max, 'walls.thickness');
}

function validatePanels(model) {
    if (typeof model.panels.profile !== 'string' || model.panels.profile.trim() === '') {
        throw new TypeError('panels.profile is required');
    }
    assertFinite(model.panels.wallHeight, 'panels.wallHeight');
    assertFinite(model.panels.wainscotHeight, 'panels.wainscotHeight');
}

function validateFoundation(model) {
    assertRange(model.foundation.height, LIMITS.foundationHeight.min, LIMITS.foundationHeight.max, 'foundation.height');
}
...
export function createBuildingModel(overrides = {}) {
    ...
    if (model.crane.zPercent === undefined && model.crane.z !== undefined) {
        model.crane.zPercent = model.crane.z;
    }
    if (model.crane.z === undefined && model.crane.zPercent !== undefined) {
        model.crane.z = model.crane.zPercent;
    }
    validate(model);
    return freeze(model);
}

export function updateBuildingModel(current, changes = {}) {
    if (!current || typeof current !== 'object') {
        throw new TypeError('current building model is required');
    }
    return createBuildingModel(merge(current, changes));
}
