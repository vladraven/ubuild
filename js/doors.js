...
const panelMat = new THREE.MeshStandardMaterial({ color: 0xdddddd, roughness: 0.8 });
const trimMat = new THREE.MeshStandardMaterial({ color: 0x334155, roughness: 0.5 });
...
export function buildDoorMesh(op) {
    ...
    const def = openingDefs[op.type] || { w: 2.0, h: 2.1 };
    const w = op.w || def.w;
    const h = op.h || def.h;
    const d = 0.2;
    const f = 0.05;
...
export function createDoorsGroupForWall(side, wallLength) {
