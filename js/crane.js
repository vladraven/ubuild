...
import { craneMat, craneRailMat } from './colorise.js';
...
export function createCraneGroup(geometry, enabled, zPercent) {
    ...
    const leftRail = new THREE.Mesh(railBeamGeo, craneRailMat);
    leftRail.position.set(craneData.rails.left.x, craneData.rails.left.y, craneData.rails.left.z);
    leftRail.castShadow = true;
    group.add(leftRail);

    const rightRail = new THREE.Mesh(railBeamGeo, craneRailMat);
    ...
    const bridgeMesh = new THREE.Mesh(bridgeGeo, craneMat);
    ...
    const trolleyMesh = new THREE.Mesh(trolleyGeo, craneRailMat);
    ...
    const cableMesh = new THREE.Mesh(cableGeo, craneRailMat);
