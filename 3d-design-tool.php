...
function get_color_hex_by_name($color_name, $all_colors)
{
    if (!$color_name) {
        return null;
    }
    ...
    foreach ($all_colors as $color) {
        if (
            isset($color['color_name']) &&
            strcasecmp(
                trim((string) $color['color_name']),
                $color_name
            ) === 0
        ) {
            $hex = trim((string) ($color['color_hexcode'] ?? ''));
...
$initial_model_colors = [
    'roof' => get_color_hex_by_name(
        get_color_defaults_optimized(
            ['Roof'],
            $color_defaults
        ),
        $all_colors
    ...
    'eaveTrim' => get_color_hex_by_name(
        get_color_defaults_optimized(
            ['Trim'],
            $color_defaults
        ),
    ...
    'rakeTrim' => get_color_hex_by_name(
        get_color_defaults_optimized(
            ['Trim'],
            $color_defaults
        ),
    ...
    'wainscot' => get_color_hex_by_name(
        get_color_defaults_optimized(
            ['Wainscot'],
            $color_defaults
        ),
    ...
    'interiorWall' => get_color_hex_by_name(
        get_color_defaults_optimized(
            ['Wall'],
            $color_defaults
        ),
...
$initial_model_colors = array_filter(
    $initial_model_colors,
...
<div style="display: none;">
                        ...
                        <div class="awning-group">
                            ...
                            <div id="ltSettingsF" style="display:none;" class="mini-input awning-settings-inner mb-3 border-start ps-2">
                                ...
                                <div class="d-flex align-items-center mb-1 justify-content-between">
                                    <span class="awning-label-width text-danger">Off R:</span>
                                    <input type="range" class="form-range lt-val dist-slider w-50" data-target="ltCutRF_val" data-side="F" data-prop="cutR" id="ltCutRF" data-m-min="0" data-m-max="50" data-m-step="0.5" data-current-m="0" value="0">
            ...
            <div class="modal-body p-4">
                <div class="row g-4">
                    ...
                    <div class="col-lg-4 ps-lg-4" id="summary-sidebar">
                        ...
                        <div class="card p-3 mb-3 border text-center" id="custom-dropzone" style="background: #ffffff; border-style: dashed !important; border-radius: 6px; cursor: pointer; transition: background-color 0.2s;">
...
window.ConfiguratorData = {
    ...
    colors: <?php echo wp_json_encode($initial_model_colors); ?>
};
</script>
